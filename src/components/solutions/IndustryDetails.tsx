...
export const IndustryDetails = ({
  ...
  return (
    <motion.div
      ...
      className="mt-8"
    >
      <div className="rounded-2xl glass-card border border-white/10 p-8 mb-12">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          >
            <div>
              <h3 className="text-xl font-bold mb-4 flex items-center text-white">
                <span className={accentColor}>Key Features</span>
              </h3>
              <div className="space-y-6">
                {features.map((feature, index) => (
                  <div key={index} className="glass-card p-5">
                    <h4 className={`font-semibold mb-2 ${accentColor}`}>{feature.title}</h4>
          ...
          >
            <div>
              <h3 className="text-xl font-bold mb-4 flex items-center text-white">
                <span className={accentColor}>Benefits</span>
              </h3>
              <div className="glass-card p-6">
                <ul className="space-y-3">
                  {benefits.map((benefit, index) => (
                    <li key={index} className="flex items-start">
                      <FiCheck className={`${accentColor} mt-1 mr-3 flex-shrink-0`} />
